...
const ContentTrackerPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedItem) {
        await axios.patch(`/api/marketing/content-tracker/${selectedItem._id}`, formData, config);
        toast.success("Content tracker item updated successfully");
      } else {
        await axios.post("/api/marketing/content-tracker", formData, config);
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {(!selectedItem || canEdit(selectedItem)) ? (
            ...
            >
              {isSaving ? "Saving..." : (selectedItem ? "Save Changes" : "Create Content Item")}
            </button>
          ) : (
            <button 
              type="button"
